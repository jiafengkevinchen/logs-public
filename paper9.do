...
global paper9_command_start "reghdfe "

*Save the arcsinh variable name (if possible, use the full variable name, not an abbreviation)
global paper9_arcsinh_var "y"

*Save the remainder of the command after the outcome variable 
global paper9_command_end " x if total >=3.25 & inter_bea   ==0 & inter_zd ==0 & inter_class==0 & inter_bea2  ==0 & inter_zd2==0 & inter_class2==0 & inter_org ~=1 & inter_org2 ~=1, absorb(year bea zd class cluster_bea_year cluster_zd_year inventor cluster1 cluster_bea_class cluster_class_year org_new) cluster(cluster1) keepsin "

*Save the main x variable (treatment) as a local
global paper9_main_xvar "x"

*Save the location of the coefficient in the paper
global paper9_location "Appendix Table 6, Column (1)"
...
use _output/paper9.dta, clear

local paper9_original_command "$paper9_command_start $paper9_arcsinh_var $paper9_command_end" 

`paper9_original_command'
display _b["$paper9_main_xvar"]



// reghdfe y x if total >=3.25 & inter_bea   ==0 & inter_zd ==0 & inter_class==0 & inter_bea2  ==0 & inter_zd2==0 & inter_class2==0 & inter_org ~=1 & inter_org2 ~=1, absorb(year bea zd class cluster_bea_year cluster_zd_year inventor cluster1 cluster_bea_class cluster_class_year org_new) cluster(cluster1) keepsin 
